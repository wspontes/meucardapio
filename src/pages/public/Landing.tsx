    ...
    <div className="min-h-screen bg-brand-black">
      <header className="border-b border-border/50">
        <div className="container mx-auto flex h-16 items-center justify-between px-4">
          <div className="flex items-center gap-2 text-xl font-bold text-brand-white">
      ...
      <section className="container mx-auto px-4 py-20 md:py-32 text-center">
        <h1 className="text-3xl md:text-6xl font-bold text-brand-white leading-tight mb-6">
      ...
      <section className="border-t border-border/50 py-16 md:py-24">
        <div className="container mx-auto px-4">
          <h2 className="text-2xl md:text-3xl font-bold text-brand-white text-center mb-12">
            Como funciona
          </h2>
            ...
            {features.map((f) => {
              ...
              return (
                <div key={f.title} className="rounded-xl border border-border bg-surface p-6 text-center">
                  ...
                  <h3 className="text-lg font-bold text-brand-white mb-2">{f.title}</h3>
          ...
          <div className="mx-auto max-w-xl space-y-6">
            {steps.map((step, i) => (
              <div key={i} className="flex items-start gap-4 rounded-xl border border-border bg-surface p-4">
                <div className="flex h-8 w-8 shrink-0 items-center justify-center rounded-full bg-accent text-sm font-bold text-brand-white">
                  {i + 1}
                </div>
                <div className="flex items-center gap-2 pt-1">
                  <CheckCircle size={18} className="text-accent shrink-0" />
                  <p className="text-brand-white">{step}</p>
                </div>
              </div>
            ))}
          </div>
